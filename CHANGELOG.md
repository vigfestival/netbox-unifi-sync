...
- **Cleanup no longer deletes management / out-of-band or other hand-managed IPs.** `cleanup_orphan_ips` previously deleted *every* unassigned IP in the tenant, so any address not currently bound to an interface — reserved IPs, gateway/management/out-of-band IPs, anything created outside the sync — was silently destroyed on each cleanup run. It now deletes an orphan only when it is genuinely sync-owned (carries the `unifi-client` tag), has `active` status, and is not a management/out-of-band address. A shared `_ip_is_mgmt_or_oob()` guard preserves any IP whose NetBox role is set, whose description/DNS/tag mentions mgmt/oob/management, or that is a device's `primary_ip4`/`primary_ip6`/`oob_ip`; the same guard now also protects the device primary-IP-change and stale-client-IP cleanup paths. (On the prod dataset this changed a sweep from deleting 75 unassigned IPs to deleting only 3 stale UniFi-client IPs.)
